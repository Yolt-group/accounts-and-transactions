...
class NotBlankAttributeSelectorDecoratorTest {

    @Test
    void shouldNotAcceptSelectorIfAttributeValueIsBlank() {
        ...
        var results =
                new NotBlankAttributeSelectorDecorator(AttributeSelectors.DESCRIPTION).selectAttribute(GeneralizedTransaction.toGeneralized(upstreamTransaction));
    ...
    @Test
    void shouldAcceptSelectorIfAttributeValueIsNotBlank() {
        var now = ZonedDateTime.now();
        var upstreamTransaction = transactionWithExternalIdAmountAndDate("a", new BigDecimal("100"), now);
        var results =
                new NotBlankAttributeSelectorDecorator(AttributeSelectors.DESCRIPTION).selectAttribute(GeneralizedTransaction.toGeneralized(upstreamTransaction));
