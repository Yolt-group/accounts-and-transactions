...
@Slf4j
@Component
public class OffloadedTransactionsConsumer {

    private final List<Message> consumed = new ArrayList<>();

    @KafkaListener(topics = "${yolt.kafka.topics.offload-yts-transactions.topic-name}",
            concurrency = "${yolt.kafka.topics.offload-yts-transactions.listener-concurrency}")
